...
//3 引入扫当前页面，调用接口拿到数据循环铺设页面
//4 点击关键词把值赋予给van-search的v-model变量

//目标：铺设搜索结果
//1 找到搜索结果接口-api/search.js定义请求方法
//2 再定义methods里getListFn方法(获取数据)
//3 在点击事件方法里调用getListFn方法拿到搜索结果的数据
//4 铺设页面（首页van-cell标签复制上来）
//5 把数据保存到data中，循环van-cell使用计科（切换歌手字段）
...
//2 一旦触底，自动执行onload方法
//3 对page++,给后台传递offset偏移量参数-请求下一页数据
...
export default {
  ...
  data() {
    return {
      ...
      hotArr: [], //热搜关键字
      resultList: [], //搜索结果
      loading: false, //加载中（状态）-只有false才自动触发Onload方法
      finished: false, //未加载全部(如果设置为true,底部就不会再次执行onload,而是加载完成)
      page: 1, //当前搜索结果的页码
    };
  },
  async created() {
  ...
  methods: {
    async getListFn() {
      ...
      //难点：async修饰的函数->默认返回一个全新Promise对象
      //这个Promise对象结果就是async函数内return的值
    ...
    async inputFn() {
      ...
      this.timer = setTimeout(async () => {
        ...
        if (this.value.length === 0) {
          //搜索关键词如果没有，把搜索结果清空阻止网络请求发送(提前return)
          this.resultList = [];
          return;
        }
        const res = await this.getListFn();
    ...
    async onLoad() {
      //触底事件（加载下一页的数据），内部会自动把loading改为true
      this.page++;
      const res = await this.getListFn();
      // console.log(res);
      if (res.data.result.songs === undefined) {
